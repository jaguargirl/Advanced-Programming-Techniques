...
namespace Lab11
{
    public partial class Form1 : Form
    {
        ...
        private OleDbDataReader rdr; private int idCda; 
        public Form1()
        {
            InitializeComponent();
        }
        ...
        private void generez_nr_cda()
        {
            ...
            cmd.Connection = con; cmd.CommandText = "SELECT Max(Comenzi.NrComanda) AS NrMax FROM Comenzi";

            con.Open(); rdr = cmd.ExecuteReader(); rdr.Read();

            txtNrComanda.Text = Convert.ToString(rdr.GetInt32(0) + 1); rdr.Close(); con.Close();
        } 
        ...
        {      
            ...
            listaValori = txtNrComanda.Text + ",#" + Convert.ToString(d.Month) + "/" + Convert.ToString(d.Day) + "/" + Convert.ToString(d.Year) + "#," + cmbClient.SelectedValue;

            cmd.CommandText = "Insert into Comenzi(" + listaCampuri + ") " + "Select " + listaValori;
        ...
        {
            cmd.CommandText = "SELECT idComanda From Comenzi Where NrComanda = " + txtNrComanda.Text;

            con.Open(); rdr = cmd.ExecuteReader(); rdr.Read();

            idCda = rdr.GetInt32(0); rdr.Close(); con.Close();
        }
        ...
        private void adaugaInregistrariComenziContinut()
        {
            ...
            con.Open(); foreach (DataRow r in dataSet1.ComandaContinutManevra)
            {
                listaValori = idCda + "," + r["NrCrt"] + "," + r["IdProdus"] + "," + r["Cantitate"] + "," + r["PretVanzare"]; cmd.CommandText = "Insert into ComenziContinut(" + listaCampuri + ") " + "Select " + listaValori;
                MessageBox.Show(cmd.CommandText); cmd.ExecuteNonQuery();
            } con.Close();
        }
